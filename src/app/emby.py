...
class Emby:
    ...
    def add_user(
        ...
        # 验证用户模板
        template_user_id = self.get_uid_from_username(user_template)
        if not template_user_id:
            error_msg = f"用户模板 '{user_template}' 不存在，请检查 EMBY_USER_TEMPLATE 配置"
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"尝试创建 Emby 用户: {username} (第 {attempt + 1}/{max_retries} 次)")
                ...
                if response.status_code == 200:
                    ...
                    if self.change_user_password(emby_id, new_password=password):
                        logger.info(f"成功创建 Emby 用户: {username} (ID: {emby_id})")
                        return True, emby_id
                    else:
                        error_msg = "设置用户密码失败"
                ...
                # 502/503/504 错误可能是临时的，值得重试
                elif response.status_code in [502, 503, 504]:
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"Emby API 返回 {response.status_code} 错误，"
                        ...
                        error_msg = f"Emby API 返回错误 ({response.status_code})，已重试 {max_retries} 次"
                        # 只记录响应的前200个字符
                        response_text = response.text[:200] if len(response.text) > 200 else response.text
                        logger.error(f"创建用户失败: {username}, {error_msg}, 响应: {response_text}...")
                        return False, error_msg
                else:
                    # 其他错误不重试
                    error_msg = f"Emby API 返回错误 ({response.status_code}): {response.text[:200]}"
                    logger.error(f"创建用户失败: {error_msg}")
                    return False, error_msg

            except requests.exceptions.Timeout:
                if attempt < max_retries - 1:
                    logger.warning(f"连接 Emby 超时，{retry_delay} 秒后重试 ({attempt + 1}/{max_retries})")
                    import time
                    time.sleep(retry_delay)
                    retry_delay *= 1.5
                    continue
                else:
                    error_msg = f"连接 Emby 服务器超时，已重试 {max_retries} 次"
                    logger.error(f"创建用户超时: {username}")
                    return False, error_msg
                    
            except requests.exceptions.ConnectionError:
                error_msg = "无法连接到 Emby 服务器，请检查网络或配置"
                logger.error(f"连接 Emby 失败: {username}")
                return False, error_msg
                
            except Exception as e:
                error_msg = f"创建用户时发生异常: {str(e)}"
                logger.error(f"创建用户异常: {username}, {error_msg}")
                return False, error_msg
        
        # 如果所有重试都失败
        return False, f"创建用户失败，已重试 {max_retries} 次"
    ...
    def get_user_total_play_time(self) -> dict[str, str]:
        ...
        # user_usage_stats 插件只在主服务器上，使用配置的 EMBY_ADMIN_URL
        # 如果未配置，则回退到 EMBY_BASE_URL
        admin_url = settings.EMBY_ADMIN_URL or self.base_url
        logger.debug(f"user_usage_stats API 使用服务器: {admin_url}")

        try:
            response = requests.post(
                url=admin_url + "/user_usage_stats/submit_custom_query",
                params=params,
                headers=headers,
                data=json.dumps(data),
                timeout=10,
            ...
            if response.status_code != 200:
                logger.error(
                    f"Emby user_usage_stats API 返回错误状态码: {response.status_code}, "
            ...
            try:
                ...
                logger.error(
                    f"Emby user_usage_stats API 返回非 JSON 响应: {e}, "
                    f"响应内容: {response.text[:200]}"
                )
                logger.warning(
                    "提示: user_usage_stats 插件可能未安装或未启用，"
                    "请确保主 Emby 服务器已安装该插件"
                )
                return {}
            ...
            logger.error(f"Emby user_usage_stats API 请求失败: {e}")
            return {}
    ...
    def add_user_library(self, user_id, library=None):
        headers = {"accept": "application/json", "Content-Type": "application/json"}
        params = {"api_key": self.api_token}
        ...
        try:
            ...
            for lib_name in library:
                ...
                if not lib:
                    logger.warning(f"资料库 '{lib_name}' 不存在，跳过")
                    continue
            ...
            if response.status_code in [200, 204]:
                ...
                error_msg = f"更新用户资料库权限失败 ({response.status_code}): {response.text}"
                logger.error(error_msg)
                return False, error_msg
            ...
            error_msg = "无法连接到 Emby 服务器"
            logger.error(error_msg)
            return False, error_msg
        except Exception as e:
            error_msg = f"添加用户资料库权限时发生异常: {str(e)}"
            logger.error(error_msg)
            return False, error_msg

    def remove_user_library(self, user_id, library=None):
        headers = {"accept": "application/json", "Content-Type": "application/json"}
        params = {"api_key": self.api_token}
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                error_msg = f"移除用户资料库权限失败 ({response.status_code}): {response.text}"
                logger.error(error_msg)
                return False, error_msg
            ...
            error_msg = f"移除用户资料库权限时发生异常: {str(e)}"
            logger.error(error_msg)
            return False, error_msg
    ...
    def delete_user(self, user_id: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                logger.info(f"Successfully deleted Emby user: {user_id}")
                return True, "ok"
            else:
            ...
            error_msg = f"Error deleting Emby user: {str(e)}"
            logger.error(error_msg)
            return False, error_msg
            user_id = user.get("Id")
