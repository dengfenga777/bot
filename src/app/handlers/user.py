...
async def exchange(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 生成邀请码
    _code = generate_unique_invitation_code(_db.invitation_code_exists)
    # 更新数据库
    # > 先更新邀请码
    ...
    await send_message(
        chat_id=chat_id,
        text="""信息: 生成邀请码成功，邀请码为 `{}`，{} 天内有效""".format(
            _code, INVITATION_EXPIRE_DAYS
        ),
        parse_mode="markdown",
        context=context,
    )
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    _invited_count = _db.get_invited_count_by_owner(chat_id)
    _db.close()
...
<strong>邀请人数: </strong>{_invited_count}
<strong>可用邀请码：</strong>
{_codes}
{"=" * 44}
