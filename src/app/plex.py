...
class Plex:
    ...
    def _authenticate_user_by_token(self, token: str) -> tuple[bool, Optional[int]]:
        ...
        try:
            ...
            logger.error(f"获取Plex用户ID失败: {e}")
            return False, None

    def remove_friend(self, user_id: int) -> tuple[bool, str]:
        """删除Plex好友（移除用户访问权限）
        ...
        """
        try:
            user = self.my_plex_account.user(user_id)
            self.my_plex_account.removeFriend(user)
            logger.info(f"Successfully removed Plex friend: {user_id}")
            return True, "ok"
        except Exception as e:
            error_msg = f"Error removing Plex friend: {str(e)}"
