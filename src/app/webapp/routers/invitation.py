...
from app.invitation_utils import (
...
async def generate_invite_code(
    ...
    try:
        ...
        # 生成邀请码
        invite_code = generate_unique_invitation_code(_db.invitation_code_exists)
        ...
        return GenerateInviteCodeResponse(
            success=True,
            message=(
                f"邀请码生成成功！已消耗 {required_points} 积分，"
                f"邀请码：{invite_code}，{INVITATION_EXPIRE_DAYS} 天内有效"
            ),
            code=invite_code,
        )
...
async def redeem_plex_code(
    ...
    try:
        ...
        try:
            ...
            if is_invitation_expired(res[2]):
                return RedeemResponse(success=False, message="邀请码已过期")
            code_owner = res[1]
...
async def check_privileged_invite_code(
    ...
    try:
        ...
        # 检查邀请码是否在特权列表中，且当前仍然有效
        is_privileged = code in settings.PRIVILEGED_CODES
        if is_privileged:
            _db = DB()
            try:
                res = _db.verify_invitation_code_is_used(code)
                if not res or res[0] or is_invitation_expired(res[2]):
                    is_privileged = False
            finally:
                _db.close()
...
async def redeem_invite_code_for_credits(
    ...
    try:
        ...
        try:
            ...
            if is_invitation_expired(res[2]):
                return RedeemForCreditsResponse(success=False, message="邀请码已过期")
            code_owner = res[1]
            # 获取用户当前积分
            stats_info = _db.get_stats_by_tg_id(user_id)
            if not stats_info:
