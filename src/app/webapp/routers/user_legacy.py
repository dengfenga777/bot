    ...
    try:
        ...
        try:
            ...
            if plex_info:
                ...
                user_info.plex_info = {
                    ...
                    "line": plex_line,
                    "line_url": plex_line_url,
                    "line_display_name": plex_line_display,
        ...
        try:
            ...
            if emby_info:
                ...
                user_info.emby_info = {
                    ...
                    "line": emby_line,
                    "line_url": emby_line_url,
                    "line_display_name": emby_line_display,
        ...
        try:
            ...
            if stats_info:
                ...
                user_info.medals = db.get_user_medals(tg_id)
                user_info.medal_multiplier = db.get_user_medal_multiplier(tg_id)
                logger.debug(
...
async def get_recent_activities(
    ...
    try:
        ...
        try:
            from app.plex import Plex
            from app.tautulli import Tautulli
            from app.utils.utils import get_user_total_duration

            plex_info = db.get_plex_info_by_tg_id(user_id)
            if plex_info:
                plex_id = plex_info[0]
                watched_time_init = plex_info[6]  # watched_time 字段
        ...
        try:
            ...
            if emby_info:
                emby_id = emby_info[1]
                emby_watched_time_init = emby_info[5]  # emby_watched_time 字段
...
async def bind_emby_line(
    ...
    try:
        ...
        # 记录线路切换历史（在设置新线路之前）
        old_line = emby_line
        db.record_line_switch(tg_id, emby_username, "emby", old_line, line)
...
async def bind_plex_line(
    ...
    try:
        ...
        # 记录线路切换历史（在设置新线路之前）
        old_line = plex_line
        db.record_line_switch(tg_id, plex_username, "plex", old_line, line)
...
async def get_all_users(
    ...
    db = DB()
    try:
        # 只获取有 Plex 或 Emby 账号的用户
        stats_users = db.cur.execute(
            """SELECT DISTINCT s.tg_id, s.donation, s.credits
            FROM statistics s
            WHERE EXISTS (
        ...
        logger.info(
            f"用户 {get_user_name_from_tg_id(user.id)} 获取了 {len(user_list)} 个有账号的用户信息"
        )
        return user_list
